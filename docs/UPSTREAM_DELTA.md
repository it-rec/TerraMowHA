...
- **Interactive map card** (`map_card.py` + `frontend/terramow-map-card.js`):
  ...
  reset, keyboard zone cycling, a season heatmap view (finished cycles per
  cell, revealing systematically missed patches), and a card editor with rotation presets and a
