...
| **Device tracker** (`device_tracker.py`) | The mower's live pose projected onto real coordinates through a configured anchor, so it appears on Home Assistant's map and works with zones/proximity. |
...
| Sensor | Per-zone **last mowed** timestamps (one entity per map sub-region, stamped from poses observed inside the boundary, with the cycle coverage as an attribute), blade / base-station **service forecasts** whose rate is measured from the device's own counter readings (and refuse to answer until the observation supports it), a default-disabled pose sensor, a **Wi-Fi signal** sensor (dp_109, mower-side strength in percent), a session-level **Active Job** sensor that reports the running mission across mid-session heartbeat gaps, and a **Fault** sensor carrying the active fault as readable text (dp_116/dp_115 resolved through the `error_codes.py` catalog). |
