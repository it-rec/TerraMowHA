...
| 150 | 高级设置（部分在设备上可写；此处只读呈现） | `{"enable_cliff_detection":{"value":true},"enable_slope_detection":{"value":false},"rain_sensor_threshold":{"upper_limit":1000},"after_rain_stop_setting":{"enable_auto_resume":false,"auto_resume_delay_time":{"hours":2,"minutes":0}},…}` | **跌落检测** / **坡度检测** / **雨后自动恢复** / **强制单一基站** / **强制蜂窝网络**二元传感器；**雨量传感器阈值** / **雨后恢复延迟**传感器（`mow_spacing`/`mow_speed` 通过 dp_155 提供；样本中 `disable_wifi_*` 被截断，未予呈现） |
