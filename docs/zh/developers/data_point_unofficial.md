...
| 152 | 环境 / 状态。多数字段是设备自行计算的状态，但 `is_illuminate_light_on` 和 `is_defogger_heating` 是**执行器**：它们采用与 dp_150 相同的协商方式写入，只有一处刻意的差别 —— **不**提供 merged-block 候选，因为把这个块原样回写会连同日出/日落和手动建图标志一起写入。因此候选结构为 `nested_field` → `wrapped_field`，并以设备自身的后续上报作为校验。**确定性的实地发现（V1000，home_assistant 模块固件）：**两个候选都被静默丢弃，与 dp_150 及 dp_122 日程写入完全一致 —— 切换**照明**灯实体后，只读的**照明**二元传感器始终停留在 `off`，也就是说**当前固件同样未开放本地 dp_152 写入**。这些写入实体（默认禁用）保留下来以备接受写入的固件，并且会如实抛出错误，而不是报告一次并未发生的写入 | `{"is_defogger_heating":false,"is_illuminate_light_on":false,"sunrise":{"hour":5,"minute":29},"sunset":{…},"is_not_in_daylight_period":false,"manual_mapping":{…}}` *只读：* **日出** / **日落**传感器；**除雾加热** / **照明** / **白昼**二元传感器；**手动建图：重新定位 / 接管 / 边界已闭合**二元传感器。*可写（默认禁用）：* **照明** `light` 实体和**除雾加热**开关 |
