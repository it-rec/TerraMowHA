...
| 150 | Advanced settings. Read-only readouts plus an **undocumented write path**, negotiated per firmware like the dp_122 schedule writes: candidate payload shapes are sent least-destructive first (`merged_block`, which echoes the whole reported block with one field replaced → `nested_field` → `flat_field` → `wrapped_field`), and a write only counts as successful when the device *reports the requested value back* within `ADVANCED_SETTING_VERIFY_TIMEOUT` — dp_119 acks are not trustworthy here (see dp_119). The proven shape is remembered per hub and tried first afterwards. If no shape is confirmed, the service call raises `advanced_setting_write_failed` instead of silently "succeeding". **Conclusive field finding (V1000, HA-module firmware):** like the dp_122 schedule writes, every candidate shape (`merged_block`, `nested_field`, `wrapped_field`) is silently dropped — tested both docked/hibernating and awake while actively mowing, with the read-only dp_150 sensors as ground truth: they never moved. So **local dp_150 writes are not exposed by current firmware**; the write entities stay in place (disabled by default) for firmware that does accept them, and correctly raise rather than report a write that did not happen | `{"enable_cliff_detection":{"value":true},"enable_slope_detection":{"value":false},"rain_sensor_threshold":{"upper_limit":1000},"after_rain_stop_setting":{"enable_auto_resume":false,"auto_resume_delay_time":{"hours":2,"minutes":0}},…}` | *Read-only:* **Cliff detection** / **Slope detection** / **After-rain auto-resume** / **Force single base station** / **Force cellular network** binary sensors; **Rain sensor threshold** / **After-rain resume delay** sensors. *Writable (config category, disabled by default):* **Cliff detection** / **Slope detection** / **After-rain auto-resume** switches and **Rain sensor threshold** / **After-rain resume delay** numbers (`mow_spacing`/`mow_speed` come via dp_155; `disable_wifi_*` truncated in the sample, not surfaced) |
| 152 | Environment / status. Mostly device-computed state, but `is_illuminate_light_on` and `is_defogger_heating` are **actuators**: they are written with the same negotiation as dp_150, with one deliberate difference — the merged-block candidate is **not** offered, because echoing this block back would write sunrise/sunset and the manual-mapping flags too. Candidates are therefore `nested_field` → `wrapped_field`, verified against the device's own follow-up report. **Conclusive field finding (V1000, HA-module firmware):** both candidates are silently dropped, exactly like dp_150 and the dp_122 schedule writes — toggling the **Illumination** light left the read-only **Illumination** binary sensor at `off`, i.e. **local dp_152 writes are not exposed by current firmware** either. The write entities stay (disabled by default) for firmware that does accept them, and raise rather than report a write that did not happen | `{"is_defogger_heating":false,"is_illuminate_light_on":false,"sunrise":{"hour":5,"minute":29},"sunset":{…},"is_not_in_daylight_period":false,"manual_mapping":{…}}` | *Read-only:* **Sunrise** / **Sunset** sensors; **Defogger heating** / **Illumination** / **Daylight** binary sensors; **Manual mapping: relocation / takeover / boundary-closed** binary sensors. *Writable (disabled by default):* the **Illumination** `light` entity and the **Defogger heating** switch |
