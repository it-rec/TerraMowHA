...
| 119 | Command acknowledgement — echoes a command's `seq` with `code:0` (= OK) or a non-zero error code; the generic per-command ack channel for dp_103/105/106/122/… writes | `{"seq":1783335426,"code":0}` | **Confirmed commands**: `terramow.start_select_region` (and the map card's tap-to-mow) waits for the ack and surfaces rejections; rejected fire-and-forget commands log a warning; last ack in diagnostics |
